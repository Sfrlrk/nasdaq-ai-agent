...
| T-003 | Test altyapısı kurulumu (network kısıtı nedeniyle lightweight harness) | Yüksek | M | Done | T-002 | Temel test altyapısı |
...
| T-006 | API katmanında retry/backoff + cache TTL (SWR benzeri) | Yüksek | M | Done | - | Rate limit dayanıklılığı |
...
| T-016 | Tarama sırasında anlık geçmiş kaydı (her başarılı sonuç) | Orta | S | Done | T-006 | Sonuçlar bitmeden history güncellenir |
...
## 6) Uygulanan Task Güncellemesi

- ✅ T-001: Legacy kök `App.jsx` dosyası shim hale getirildi; tek kaynak `src/App.jsx`.
- ✅ T-003: Paket indirimi kısıtlı ortamda çalışacak lightweight test harness eklendi (`scripts/run-tests.mjs`).
- ✅ T-006: Yahoo fetch katmanına cache TTL + retry/backoff eklendi.
- ✅ T-011: Hisse detayında yeni `Skor Açıklaması` paneli eklendi (bull/bear sinyal ağırlıkları).

- ✅ T-016: Tarama sırasında her başarılı sonuç için anlık geçmiş kaydı eklendi (`saveLiveResult`).
